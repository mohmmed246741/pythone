    ...
    def train(self,batch):
        ...
        for epoch in range(0, 1):
            ...
            for i in range(total_train_batch_num):
                ...
                print(images.shape)
                # s = 233
                # image2 = Image.fromarray(images[s, :, :].reshape(32, 32))
                #
                # image2.show()

                gt_class = np.asarray(labels)
                #################### training

                X_train_batch=images

                X_train_batch=X_train_batch.reshape(-1,32,32,1)
                print(gt_class.shape)
                print(X_train_batch.shape)



                y_class=self.sess.run(self.Y_class,feed_dict={self.X: X_train_batch})

                print(y_class.shape)


                new_y_class = np.zeros((3360))

                #
                # print('y_gt: '+str(gt_class[s]))
                # print('y_pred: '+str(np.argmax(y_class[s,:])))
                #
                ...
                for i in range(3360):
                    new_y_class[i] = (np.argmax(y_class[i,:] ))

                new_y_class=new_y_class.astype(int)

                print('y_gt: ' + str(gt_class[0]))
                print(new_y_class[0])

                c=confusion_matrix(gt_class, new_y_class)
                confusion_matrix1 = pd.DataFrame(c)
